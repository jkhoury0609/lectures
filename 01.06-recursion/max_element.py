def max_element(lst):
    """
    Recursion is the definition of a value in terms of itself, and a recursive
     functions is one that calls itself. A function ought to take as arguments
     an instance of a problem, and produce as return value a solution to that
     problem. Thus, a recursive function must identify a smaller version of the
     *same* problem.
    ...
    #  elements. They have two different solutions, so they require two
    #  different cases:
    if len(lst) == 0:
        return None
    elif len(lst) == 1:
